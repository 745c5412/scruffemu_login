...
public class Config
{
  ...
  private static void load(String name)
  {
    ...
    try
    {
      Config.exchangeIp=properties.getProperty(Params.EXCHANGE_IP.toString());
      i=6;
      Config.exchangePort=Integer.parseInt(properties.getProperty(Params.EXCHANGE_PORT.toString()));
      i=9;
      Config.loginPort=Integer.parseInt(properties.getProperty(Params.PORT.toString()));
      i++;
      Config.version=properties.getProperty(Params.VERSION.toString());
      i=12;
      Config.host=properties.getProperty(Params.LOGIN_DB_HOST.toString());
      i++;
      Config.port=Integer.parseInt(properties.getProperty(Params.LOGIN_DB_PORT.toString()));
      i++;
      Config.user=properties.getProperty(Params.LOGIN_DB_USER.toString());
      i++;
      Config.pass=properties.getProperty(Params.LOGIN_DB_PASS.toString());
      i++;
      Config.databaseName=properties.getProperty(Params.LOGIN_DB_NAME.toString());
      i++;
  ...
  private enum Params {
    EXCHANGE_PORT("system.server.exchange.port"), EXCHANGE_IP("system.server.exchange.ip"), PORT("system.server.login.port"), VERSION("system.server.login.version"), LOGIN_DB_HOST(
        "database.login.host"), LOGIN_DB_PORT(
            "database.login.port"), LOGIN_DB_USER("database.login.user"), LOGIN_DB_PASS("database.login.pass"), LOGIN_DB_NAME("database.login.name"), LOGIN_WEB_URL("web.url");
